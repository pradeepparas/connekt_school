        ...
        this.setState({
            ...
            () => {
              if(this.state.count){
                ...
                this.getSession(this.state.current_page)
              }
            });
      }
...
/// ON SEARCH COURSE
onSearchCourese=()=>{
this.getSession(1, this.state.current_page, this.state.classId, this.state.status)
}
...
deleteSessionById = async(e) => {
  debugger
  e.preventDefault();
  //http://35.200.220.64:4000/connektschool/deleteSession?status=0&SessionId=1
  this.setState({isLoading:true})
//   {
//     "status": "200",
//     "error": true,
...
// }
  fetch(api_Url+`deleteSession?status=0&SessionId=${this.state.id}`,{
   method:"GET",
   headers :{
     "Accept":"Application/json",
     "Content-Type":"Application/json",
 ...
 .then(result=>{
   ...
   if(result.error){
     ...
     this.setState({id:""},()=>{
      this.getSession(this.state.current_page, this.state.per_page)
...
const response = await fetch( api_Url+`getSessionBySchoolId?page=${pageNumber}&size=${this.state.per_page}&status=${value}`,{
    method: "GET",
    headers: {
      "Accept": "application/json",
      "Content-Type": "application/json",
...
validateForm =()=>{
  debugger
   var isValid=true;
    if(this.state.classId==''){
        isValid= false
        return this.setState({classId_ErMsg:"Class name is required", sessionName_ErMsg:"", courseOtherDetails_ErMsg:"", courseDescription_ErMsg:"", courseImage_ErMsg:""})
    }
   else if(this.state.sessionName.trim()==''){
       isValid= false
        return this.setState({classId_ErMsg:"", sessionName_ErMsg:"Course name is required", courseOtherDetails_ErMsg:"", courseDescription_ErMsg:"", courseImage_ErMsg:""})
    }
    else if(this.state.courseDescription.trim()==''){
        isValid= false
        return this.setState({classId_ErMsg:"", sessionName_ErMsg:"", courseOtherDetails_ErMsg:"", courseDescription_ErMsg:"Description is required", courseImage_ErMsg:""})
    }
    // else  if(this.state.courseOtherDetails.trim()==''){
    //     isValid= false
    //     return this.setState({classId_ErMsg:"", sessionName_ErMsg:"", courseOtherDetails_ErMsg:"Course other description is required", courseDescription_ErMsg:"", courseImage_ErMsg:""})
    // }
    // else  if(this.state.isAdd&&this.state.courseImage.length==0){
    //     isValid= false
    //     return this.setState({classId_ErMsg:"", sessionName_ErMsg:"", courseOtherDetails_ErMsg:"", courseDescription_ErMsg:"", courseImage_ErMsg:"Course image is required"})
    // }
    else{
        return isValid
    }
          ...
          .then(result=>{
            ...
            if(result.success){
              ...
              this.setState({classId:""},()=>{
                this.getSession(this.state.current_page, this.state.per_page, this.state.classId, this.state.status)
  ...
  showSession = () => {
    if (this.state.sessionList !== undefined) {
      return this.state.sessionList.map((session, i) => {
        return (
          <tr>
            <td>{((this.state.current_page - 1) * this.state.per_page) + (i + 1)}</td>
            <td>{session.SessionName}</td>
            {/*<td>{session.CourseOtherDetails}</td>*/}
            ...
            <td>
            <i  onClick={() => this.editSession(session)} class="ti-pencil"></i>
            {" "}  {" "}
            {<i  onClick={() => this.deleteSession(session)} class="ti-trash"></i>}
            </td>
      ...
      if (
        ...
        return (
          <span
            key={number}
            className={classes}
            onClick={() => this.getSession(number, this.state.per_page, this.state.classId, this.state.status, this.state.searchStr,)}
          >
            {number}
          </span>
        );
...
uploadFile=(e, type, i)=>{
  ...
  render() {
    return (
      ...
      <SideBar tabIndex='session'  shown='master' />
              ...
              <div className="row align-items-center">
                <div className="col-sm-6">
                  <div className="breadcrumbs-area clearfix">
                    <h4 className="page-title pull-left">Session</h4>
                  ...
                  <div class="card">
                    <div class="card-body">
                      ...
                      <div class="single-table">
                        <div class="table-responsive">
                          <table class="table text-center">
                            ...
                            <tbody>{this.showSession()}</tbody>


                          </table>
                          {this.state.sessionList.length == 0 && <p style={{ textAlign: "center" }}> No Record Found</p>}
                      ...
                      <span className={this.state.current_page=='1'?"disabled":""}
                        onClick={()=> {
                          ...
                          this.getSession(this.state.current_page-1, this.state.per_page, this.state.searchStr)}}>Previous</span>
                        {this.state.sessionList.length > 0 && this.renderPageNumbers()}
                    <span  className={Math.ceil(this.state.total / this.state.per_page)==this.state.current_page?"disabled":""}
                        onClick={()=> {
                          ...
                          this.getSession(+this.state.current_page+1, this.state.per_page, this.state.searchStr)}}>Next</span>
                   </div>}
                        </div>
                      </div>
                    </div>
		...
		<Modal show={this.state.show} backdrop="static" onHide={this.handleClose}>
            ...
            <div>
						...
						<Col sm={9}>
							<FormControl type="text" placeholder="Session Name like(2019-20)" value={this.state.sessionName} onChange={this.handleInputs} name="sessionName" />
			...
			<Modal.Footer>
				...
				<Button type="submit" disabled={this.state.isValiddata} onClick={this.manageSession} bsStyle="primary"> {" "} {" "} {this.state.btntitle} </Button>
			</Modal.Footer>
		</Modal>
	</div>
   {/* Delete modal */}
	...
	<div>
		<Modal show={this.state.showDeleteModal} onHide={this.handleDeleteClose}>
			...
			<Modal.Body>
				<Form horizontal>
					<FormGroup controlId="formHorizontalEmail">
						...
						<Col sm={9}> Are you sure you want to delete <strong>{this.state.sessionName}</strong> ?
			...
			<Modal.Footer>
				...
				<Button type="submit" onClick={this.deleteSessionById} bsStyle="primary"> {" "} {" "} {this.state.btntitle} </Button>
			</Modal.Footer>
		</Modal>
	</div>
</div>

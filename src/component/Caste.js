        ...
        this.setState({
            ...
            () => {
              if(this.state.count){
                ...
                this.getCaste(this.state.current_page)
              }
            });
      }
...
/// ON SEARCH COURSE
onSearchCourese=()=>{
this.getCaste(1, this.state.current_page, this.state.classId, this.state.status)
}
...
deleteCasteById = async(e) => {
  debugger
  e.preventDefault();
  //http://35.200.220.64:4000/connektschool/deleteCaste?status=0&CasteMasterId=1
  this.setState({isLoading:true})
//   {
//     "status": "200",
//     "error": true,
...
// }
  fetch(api_Url+`deleteCaste?status=0&CasteMasterId=${this.state.id}`,{
   method:"GET",
   headers :{
     "Accept":"Application/json",
     "Content-Type":"Application/json",
 ...
 .then(result=>{
   ...
   if(result.error){
     ...
     this.setState({id:""},()=>{
      this.getCaste(this.state.current_page, this.state.per_page)
...
const response = await fetch( api_Url+`getCasteBySchoolId?page=${pageNumber}&size=${this.state.per_page}&status=${value}`,{
    method: "GET",
    headers: {
      "Accept": "application/json",
      "Content-Type": "application/json",
...
validateForm =()=>{
  debugger
   var isValid=true;
    if(this.state.classId==''){
        isValid= false
        return this.setState({classId_ErMsg:"Class name is required", casteName_ErMsg:"", courseOtherDetails_ErMsg:"", courseDescription_ErMsg:"", courseImage_ErMsg:""})
    }
   else if(this.state.casteName.trim()==''){
       isValid= false
        return this.setState({classId_ErMsg:"", casteName_ErMsg:"Course name is required", courseOtherDetails_ErMsg:"", courseDescription_ErMsg:"", courseImage_ErMsg:""})
    }
    else if(this.state.courseDescription.trim()==''){
        isValid= false
        return this.setState({classId_ErMsg:"", casteName_ErMsg:"", courseOtherDetails_ErMsg:"", courseDescription_ErMsg:"Description is required", courseImage_ErMsg:""})
    }
    // else  if(this.state.courseOtherDetails.trim()==''){
    //     isValid= false
    //     return this.setState({classId_ErMsg:"", casteName_ErMsg:"", courseOtherDetails_ErMsg:"Course other description is required", courseDescription_ErMsg:"", courseImage_ErMsg:""})
    // }
    // else  if(this.state.isAdd&&this.state.courseImage.length==0){
    //     isValid= false
    //     return this.setState({classId_ErMsg:"", casteName_ErMsg:"", courseOtherDetails_ErMsg:"", courseDescription_ErMsg:"", courseImage_ErMsg:"Course image is required"})
    // }
    else{
        return isValid
    }
   ...
   if(this.state.isAdd){
    ...
    this.setState({isLoading:true})
         fetch(api_Url+url,{
          ...
          .then(result=>{
            ...
            if(result.success){
              ...
              this.setState({classId:""},()=>{
                this.getCaste(this.state.current_page, this.state.per_page, this.state.classId, this.state.status)
          ...
          var data = {
              "CasteMasterId":this.state.id,
              "CasteName": this.state.casteName,
              "StatusId":this.state.active?"1":"0"
              }
          this.setState({isLoading:true})

          fetch(api_Url+`updateCaste`,{
            method:"POST",
            headers :{
              "Accept":"Application/json",
              "Content-Type":"Application/json",
  ...
  showCaste = () => {
    if (this.state.casteList !== undefined) {
      return this.state.casteList.map((section, i) => {
        return (
          <tr>
            <td>{((this.state.current_page - 1) * this.state.per_page) + (i + 1)}</td>
            <td>{section.CasteName}</td>
            ...
            <td>
            <i  onClick={() => this.editCaste(section)} class="ti-pencil"></i>
            {" "}  {" "}
            {<i  onClick={() => this.deleteCaste(section)} class="ti-trash"></i>}
            </td>
      ...
      if (
        ...
        return (
          <span
            key={number}
            className={classes}
            onClick={() => this.getCaste(number, this.state.per_page, this.state.classId, this.state.status, this.state.searchStr,)}
          >
            {number}
          </span>
        );
    ...
    return (
      ...
      <SideBar tabIndex='section'  shown='master' />
              ...
              <div className="row align-items-center">
                <div className="col-sm-6">
                  <div className="breadcrumbs-area clearfix">
                    <h4 className="page-title pull-left">Caste</h4>
              ...
              <div class="row">
                ...
                <div class="col-lg-12 mt-5">
                  ...
                  <div class="card">
                    <div class="card-body">
                      <div className="">
                        <h4 class="header-title">Caste List</h4>
                      ...
                      <div class="single-table">
                        <div class="table-responsive">
                          <table class="table text-center">
                            ...
                            <tbody>{this.showCaste()}</tbody>


                          </table>
                          {this.state.casteList.length == 0 && <p style={{ textAlign: "center" }}> No Record Found</p>}


                        </div>
                        {this.state.casteList.length>0&&
                      <div className={styles.pagination}>
                      <span className={this.state.current_page=='1'?"disabled":""}
                        onClick={()=> {
                          if(this.state.current_page=='1'){return;}
                          this.getCaste(this.state.current_page-1, this.state.per_page, this.state.searchStr)}}>Previous</span>
                        {this.state.casteList.length > 0 && this.renderPageNumbers()}
                    <span  className={Math.ceil(this.state.total / this.state.per_page)==this.state.current_page?"disabled":""}
                        onClick={()=> {
                          ...
                          this.getCaste(+this.state.current_page+1, this.state.per_page, this.state.searchStr)}}>Next</span>
                   </div>}
                        </div>
                      </div>
                    </div>
		...
		<Modal show={this.state.show} backdrop="static" onHide={this.handleClose}>
            ...
            <div>
						<Col sm={4}> Caste Name <small style={{color: 'red', fontSize: 18}}>*</small></Col>
						<Col sm={9}>
							<FormControl type="text" placeholder="Caste Name" value={this.state.casteName} onChange={this.handleInputs} name="casteName" />
							<small className={this.state.displaytext + " text-danger"}>{this.state.casteName_ErMsg}</small>
						</Col>
            <Col sm={9}>
			...
			<Modal.Footer>
				...
				<Button type="submit" disabled={this.state.isValiddata} onClick={this.manageCaste} bsStyle="primary"> {" "} {" "} {this.state.btntitle} </Button>
			</Modal.Footer>
		</Modal>
	</div>
   {/* Delete modal */}
	...
	<div>
		<Modal show={this.state.showDeleteModal} onHide={this.handleDeleteClose}>
			...
			<Modal.Body>
				<Form horizontal>
					<FormGroup controlId="formHorizontalEmail">
						...
						<Col sm={9}> Are you sure you want to delete <strong>{this.state.casteName}</strong> ?
			...
			<Modal.Footer>
				...
				<Button type="submit" onClick={this.deleteCasteById} bsStyle="primary"> {" "} {" "} {this.state.btntitle} </Button>
			</Modal.Footer>
		</Modal>
	</div>
</div>

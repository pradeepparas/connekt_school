        ...
        this.setState({
            ...
            () => {
              if(this.state.count){
                ...
                this.getReligion(this.state.current_page)
              }
            });
      }
...
/// ON SEARCH COURSE
onSearchCourese=()=>{
this.getReligion(1, this.state.current_page, this.state.classId, this.state.status)
}
...
deleteReligionById = async(e) => {
  debugger
  e.preventDefault();
  //http://35.200.220.64:4000/connektschool/deleteReligion?status=0&ReligionMasterId=1
...
// }
 ...
 .then(result=>{
   ...
   if(result.error){
     ...
     this.setState({id:""},()=>{
      this.getReligion(this.state.current_page, this.state.per_page)
...
validateForm =()=>{
   ...
   if(this.state.isAdd){
    ...
    this.setState({isLoading:true})
         fetch(api_Url+url,{
          ...
          .then(result=>{
            ...
            if(result.success){
              ...
              this.setState({classId:""},()=>{
                this.getReligion(this.state.current_page, this.state.per_page, this.state.classId, this.state.status)
          ...
          var data = {
              "ReligionMasterId":this.state.id,
              "ReligionName": this.state.religionName,
              "StatusId":this.state.active?"1":"0"
              }
          this.setState({isLoading:true})

          fetch(api_Url+`updateReligion`,{
            method:"POST",
            headers :{
              "Accept":"Application/json",
              "Content-Type":"Application/json",
  ...
  showReligion = () => {
    if (this.state.religionList !== undefined) {
      return this.state.religionList.map((section, i) => {
        return (
          <tr>
            <td>{((this.state.current_page - 1) * this.state.per_page) + (i + 1)}</td>
            <td>{section.ReligionName}</td>
            ...
            <td>
            <i  onClick={() => this.editReligion(section)} class="ti-pencil"></i>
            {" "}  {" "}
            {<i  onClick={() => this.deleteReligion(section)} class="ti-trash"></i>}
            </td>
      ...
      if (
        ...
        return (
          <span
            key={number}
            className={classes}
            onClick={() => this.getReligion(number, this.state.per_page, this.state.classId, this.state.status, this.state.searchStr,)}
          >
            {number}
          </span>
        );
    ...
    return (
      ...
      <SideBar tabIndex='section'  shown='master' />
              ...
              <div className="row align-items-center">
                <div className="col-sm-6">
                  <div className="breadcrumbs-area clearfix">
                    <h4 className="page-title pull-left">Religion</h4>
              ...
              <div class="row">
                ...
                <div class="col-lg-12 mt-5">
                  ...
                  <div class="card">
                    <div class="card-body">
                      <div className="">
                        <h4 class="header-title">Religion List</h4>
                      ...
                      <div class="single-table">
                        <div class="table-responsive">
                          <table class="table text-center">
                            ...
                            <tbody>{this.showReligion()}</tbody>


                          </table>
                          {this.state.religionList.length == 0 && <p style={{ textAlign: "center" }}> No Record Found</p>}


                        </div>
                        {this.state.religionList.length>0&&
                      <div className={styles.pagination}>
                      <span className={this.state.current_page=='1'?"disabled":""}
                        onClick={()=> {
                          if(this.state.current_page=='1'){return;}
                          this.getReligion(this.state.current_page-1, this.state.per_page, this.state.searchStr)}}>Previous</span>
                        {this.state.religionList.length > 0 && this.renderPageNumbers()}
                    <span  className={Math.ceil(this.state.total / this.state.per_page)==this.state.current_page?"disabled":""}
                        onClick={()=> {
                          ...
                          this.getReligion(+this.state.current_page+1, this.state.per_page, this.state.searchStr)}}>Next</span>
                   </div>}
                        </div>
                      </div>
                    </div>
		...
		<Modal show={this.state.show} backdrop="static" onHide={this.handleClose}>
            ...
            <div>
						<Col sm={4}> Religion Name <small style={{color: 'red', fontSize: 18}}>*</small></Col>
						<Col sm={9}>
							<FormControl type="text" placeholder="Religion Name" value={this.state.religionName} onChange={this.handleInputs} name="religionName" />
							<small className={this.state.displaytext + " text-danger"}>{this.state.religionName_ErMsg}</small>
						</Col>
            <Col sm={9}>
			...
			<Modal.Footer>
				...
				<Button type="submit" disabled={this.state.isValiddata} onClick={this.manageReligion} bsStyle="primary"> {" "} {" "} {this.state.btntitle} </Button>
			</Modal.Footer>
		</Modal>
	</div>
   {/* Delete modal */}
	...
	<div>
		<Modal show={this.state.showDeleteModal} onHide={this.handleDeleteClose}>
			...
			<Modal.Body>
				<Form horizontal>
					<FormGroup controlId="formHorizontalEmail">
						...
						<Col sm={9}> Are you sure you want to delete <strong>{this.state.religionName}</strong> ?
			...
			<Modal.Footer>
				...
				<Button type="submit" onClick={this.deleteReligionById} bsStyle="primary"> {" "} {" "} {this.state.btntitle} </Button>
			</Modal.Footer>
		</Modal>
	</div>
</div>

        ...
        this.setState({
            ...
            () => {
              if(this.state.count){
                ...
                this.getSection(this.state.current_page)
              }
            });
      }
...
/// ON SEARCH COURSE
onSearchCourese=()=>{
this.getSection(1, this.state.current_page, this.state.classId, this.state.status)
}
...
deleteSectionById = async(e) => {
  debugger
  e.preventDefault();
  //http://35.200.220.64:4000/connektschool/deleteSection?status=0&SectionMasterId=1
  this.setState({isLoading:true})
//   {
//     "status": "200",
//     "error": true,
...
// }
  fetch(api_Url+`deleteSection?status=0&SectionMasterId=${this.state.id}`,{
   method:"GET",
   headers :{
     "Accept":"Application/json",
     "Content-Type":"Application/json",
 ...
 .then(result=>{
   ...
   if(result.error){
     ...
     this.setState({id:""},()=>{
      this.getSection(this.state.current_page, this.state.per_page)
...
const response = await fetch( api_Url+`getSectionBySchoolId?page=${pageNumber}&size=${this.state.per_page}&status=${value}`,{
    method: "GET",
    headers: {
      "Accept": "application/json",
      "Content-Type": "application/json",
...
validateForm =()=>{
  debugger
   var isValid=true;
    ...
    //     isValid= false
    //     return this.setState({classId_ErMsg:"", sectionName_ErMsg:"", courseOtherDetails_ErMsg:"", courseDescription_ErMsg:"", courseImage_ErMsg:"Course image is required"})
    // }
    else{
        return isValid
    }
   ...
   if(this.state.isAdd){
    ...
    this.setState({isLoading:true})
         fetch(api_Url+url,{
          ...
          .then(result=>{
            ...
            if(result.success){
              ...
              this.setState({classId:""},()=>{
                this.getSection(this.state.current_page, this.state.per_page, this.state.classId, this.state.status)
          ...
          var data = {
              "SectionMasterId":this.state.id,
              "SectionName": this.state.sectionName,
              "StatusId":this.state.active?"1":"0"
              }
          this.setState({isLoading:true})

          fetch(api_Url+`updateSection`,{
            method:"POST",
            headers :{
              "Accept":"Application/json",
              "Content-Type":"Application/json",
  ...
  showSection = () => {
    if (this.state.sectionList !== undefined) {
      return this.state.sectionList.map((section, i) => {
        return (
          <tr>
            ...
            <td>
            <i  onClick={() => this.editSection(section)} class="ti-pencil"></i>
            {" "}  {" "}
            {<i  onClick={() => this.deleteSection(section)} class="ti-trash"></i>}
            </td>
      ...
      if (
        ...
        return (
          <span
            key={number}
            className={classes}
            onClick={() => this.getSection(number, this.state.per_page, this.state.classId, this.state.status, this.state.searchStr,)}
          >
            {number}
          </span>
        );
    ...
    return (
      ...
      <SideBar tabIndex='section'  shown='master' />
              ...
              <div className="row align-items-center">
                <div className="col-sm-6">
                  <div className="breadcrumbs-area clearfix">
                    <h4 className="page-title pull-left">Section</h4>
                  ...
                  <div class="card">
                    <div class="card-body">
                      ...
                      <div class="single-table">
                        <div class="table-responsive">
                          <table class="table text-center">
                            ...
                            <tbody>{this.showSection()}</tbody>


                          </table>
                          {this.state.sectionList.length == 0 && <p style={{ textAlign: "center" }}> No Record Found</p>}
                      ...
                      <span className={this.state.current_page=='1'?"disabled":""}
                        onClick={()=> {
                          ...
                          this.getSection(this.state.current_page-1, this.state.per_page, this.state.searchStr)}}>Previous</span>
                        {this.state.sectionList.length > 0 && this.renderPageNumbers()}
                    <span  className={Math.ceil(this.state.total / this.state.per_page)==this.state.current_page?"disabled":""}
                        onClick={()=> {
                          ...
                          this.getSection(+this.state.current_page+1, this.state.per_page, this.state.searchStr)}}>Next</span>
                   </div>}
                        </div>
                      </div>
                    </div>
		...
		<Modal show={this.state.show} backdrop="static" onHide={this.handleClose}>
            ...
            <div>
						<Col sm={4}> Section Name <small style={{color: 'red', fontSize: 18}}>*</small></Col>
						<Col sm={9}>
							<FormControl type="text" placeholder="Section Name" value={this.state.sectionName} onChange={this.handleInputs} name="sectionName" />
							<small className={this.state.displaytext + " text-danger"}>{this.state.sectionName_ErMsg}</small>
						</Col>
            <Col sm={9}>
			...
			<Modal.Footer>
				...
				<Button type="submit" disabled={this.state.isValiddata} onClick={this.manageSection} bsStyle="primary"> {" "} {" "} {this.state.btntitle} </Button>
			</Modal.Footer>
		</Modal>
	</div>
   {/* Delete modal */}
	...
	<div>
		<Modal show={this.state.showDeleteModal} onHide={this.handleDeleteClose}>
			...
			<Modal.Body>
				<Form horizontal>
					<FormGroup controlId="formHorizontalEmail">
						...
						<Col sm={9}> Are you sure you want to delete <strong>{this.state.sectionName}</strong> ?
			...
			<Modal.Footer>
				...
				<Button type="submit" onClick={this.deleteSectionById} bsStyle="primary"> {" "} {" "} {this.state.btntitle} </Button>
			</Modal.Footer>
		</Modal>
	</div>
</div>

        ...
        this.setState({
            ...
            () => {
              if(this.state.count){
                ...
                this.getSchoolMedium(this.state.current_page)
              }
            });
      }
...
editSchoolMedium = (data) => {
  console.log(data)
  debugger
  this.setState({
    show: true,
     insertType:"single",
    title: 'Update School Medium',
    schoolMedium:data.SchoolMedium,
    active:data.StatusId==1?true:false,
    id:data.SchoolMediumId,
    schoolMedium_ErMsg:"",
    btntitle: 'Update',
    isAdd: false,
    isEdit: true,
...
/// ON SEARCH COURSE
onSearchCourese=()=>{
this.getSchoolMedium(1, this.state.current_page, this.state.classId, this.state.status)
}


// delete section
deleteSchoolMedium = (data) => {
  console.log(data)
  debugger
   this.setState({
     id: data.SchoolMediumId,
     title: 'Delete School Medium',
     btntitle: 'Delete',
     btnValue: "Delete",
     show: false,
...
deleteSchoolMediumById = async(e) => {
  debugger
  e.preventDefault();
  //http://35.200.220.64:4000/connektschool/deleteSchoolMedium?status=0&SchoolMediumMasterId=1
  this.setState({isLoading:true})
//   {
//     "status": "200",
//     "error": true,
//     "message": "SchoolMedium Deleted."
// }
  fetch(api_Url+`deleteSchoolMedium?status=0&SchoolMediumId=${this.state.id}`,{
   method:"GET",
   headers :{
     "Accept":"Application/json",
     "Content-Type":"Application/json",
 ...
 .then(result=>{
   ...
   if(result.error){
     ...
     this.setState({id:""},()=>{
      this.getSchoolMedium(this.state.current_page, this.state.per_page)
...
getSchoolMedium = async pageNumber => {
  debugger
  this.setState({isLoading:true})
try{
  let value = '1';
...
const response = await fetch( api_Url+`getSchoolMediumBySchoolId?page=${pageNumber}&size=${this.state.per_page}&status=${value}`,{
    method: "GET",
    headers: {
      "Accept": "application/json",
      "Content-Type": "application/json",
...
validateForm =()=>{
  debugger
   var isValid=true;
    if(this.state.classId==''){
        isValid= false
        return this.setState({classId_ErMsg:"Class name is required", schoolMedium_ErMsg:"", courseOtherDetails_ErMsg:"", courseDescription_ErMsg:"", courseImage_ErMsg:""})
    }
   else if(this.state.schoolMedium.trim()==''){
       isValid= false
        return this.setState({classId_ErMsg:"", schoolMedium_ErMsg:"Course name is required", courseOtherDetails_ErMsg:"", courseDescription_ErMsg:"", courseImage_ErMsg:""})
    }
    else if(this.state.courseDescription.trim()==''){
        isValid= false
        return this.setState({classId_ErMsg:"", schoolMedium_ErMsg:"", courseOtherDetails_ErMsg:"", courseDescription_ErMsg:"Description is required", courseImage_ErMsg:""})
    }
    // else  if(this.state.courseOtherDetails.trim()==''){
    //     isValid= false
    //     return this.setState({classId_ErMsg:"", schoolMedium_ErMsg:"", courseOtherDetails_ErMsg:"Course other description is required", courseDescription_ErMsg:"", courseImage_ErMsg:""})
    // }
    // else  if(this.state.isAdd&&this.state.courseImage.length==0){
    //     isValid= false
    //     return this.setState({classId_ErMsg:"", schoolMedium_ErMsg:"", courseOtherDetails_ErMsg:"", courseDescription_ErMsg:"", courseImage_ErMsg:"Course image is required"})
    // }
    else{
        return isValid
    }
   ...
   if(this.state.isAdd){
    ...
    this.setState({isLoading:true})
         fetch(api_Url+url,{
          ...
          .then(result=>{
            ...
            if(result.success){
              ...
              this.setState({classId:""},()=>{
                this.getSchoolMedium(this.state.current_page, this.state.per_page, this.state.classId, this.state.status)
          ...
          var data = {
              "SchoolMediumId":this.state.id,
              "SchoolMedium": this.state.schoolMedium,
              "StatusId":this.state.active?"1":"0"
              }
          this.setState({isLoading:true})

          fetch(api_Url+`updateSchoolMedium`,{
            method:"POST",
            headers :{
              "Accept":"Application/json",
              "Content-Type":"Application/json",
  ...
  showSchoolMedium = () => {
    if (this.state.mediumList !== undefined) {
      return this.state.mediumList.map((section, i) => {
        return (
          <tr>
            <td>{((this.state.current_page - 1) * this.state.per_page) + (i + 1)}</td>
            <td>{section.SchoolMedium}</td>
            ...
            <td>
            <i  onClick={() => this.editSchoolMedium(section)} class="ti-pencil"></i>
            {" "}  {" "}
            {<i  onClick={() => this.deleteSchoolMedium(section)} class="ti-trash"></i>}
            </td>
      ...
      if (
        ...
        return (
          <span
            key={number}
            className={classes}
            onClick={() => this.getSchoolMedium(number, this.state.per_page, this.state.classId, this.state.status, this.state.searchStr,)}
          >
            {number}
          </span>
        );
...
uploadFile=(e, type, i)=>{
  ...
  render() {
    return (
      ...
      <SideBar tabIndex='school-medium'  shown='master' />
              ...
              <div className="row align-items-center">
                <div className="col-sm-6">
                  <div className="breadcrumbs-area clearfix">
                    <h4 className="page-title pull-left">School Medium</h4>

                    <ul className="breadcrumbs pull-left">
                      <li><a >Home</a></li>
                      <li><span>School Medium</span></li>
                    </ul>
                  </div>
                </div>
              </div>
              ...
              <div class="row">
                ...
                <div class="col-lg-12 mt-5">
                  ...
                  <div class="card">
                    <div class="card-body">
                      <div className="">
                        <h4 class="header-title">School Medium List</h4>
                          ...
                          <table class="table text-center">
                            <thead class="text-uppercase  bg-light-green">
                              <tr class="text-white">
                                ...
                                <th scope="col">School Medium Name</th>
                            ...
                            <tbody>{this.showSchoolMedium()}</tbody>


                          </table>
                          {this.state.mediumList.length == 0 && <p style={{ textAlign: "center" }}> No Record Found</p>}


                        </div>
                        {this.state.mediumList.length>0&&
                      <div className={styles.pagination}>
                      <span className={this.state.current_page=='1'?"disabled":""}
                        onClick={()=> {
                          if(this.state.current_page=='1'){return;}
                          this.getSchoolMedium(this.state.current_page-1, this.state.per_page, this.state.searchStr)}}>Previous</span>
                        {this.state.mediumList.length > 0 && this.renderPageNumbers()}
                    <span  className={Math.ceil(this.state.total / this.state.per_page)==this.state.current_page?"disabled":""}
                        onClick={()=> {
                          ...
                          this.getSchoolMedium(+this.state.current_page+1, this.state.per_page, this.state.searchStr)}}>Next</span>
                   </div>}
                        </div>
                      </div>
                    </div>
		...
		<Modal show={this.state.show} backdrop="static" onHide={this.handleClose}>
            ...
            <div>
						<Col sm={4}>Medium Name<small style={{color: 'red', fontSize: 18}}>*</small></Col>
						<Col sm={9}>
							<FormControl type="text" placeholder="Medium Name like (English/Hindi)" value={this.state.schoolMedium} onChange={this.handleInputs} name="schoolMedium" />
							<small className={this.state.displaytext + " text-danger"}>{this.state.schoolMedium_ErMsg}</small>
						</Col>
            <Col sm={9}>
			...
			<Modal.Footer>
				...
				<Button type="submit" disabled={this.state.isValiddata} onClick={this.manageSchoolMedium} bsStyle="primary"> {" "} {" "} {this.state.btntitle} </Button>
			</Modal.Footer>
		</Modal>
	</div>
   {/* Delete modal */}
	...
	<div>
		<Modal show={this.state.showDeleteModal} onHide={this.handleDeleteClose}>
			...
			<Modal.Body>
				<Form horizontal>
					<FormGroup controlId="formHorizontalEmail">
						...
						<Col sm={9}> Are you sure you want to delete <strong>{this.state.schoolMedium}</strong> ?
			...
			<Modal.Footer>
				...
				<Button type="submit" onClick={this.deleteSchoolMediumById} bsStyle="primary"> {" "} {" "} {this.state.btntitle} </Button>
			</Modal.Footer>
		</Modal>
	</div>
</div>
